...
class SearchScreen extends StatelessWidget {
  ...
  double rating = 5.0;
    ...
    Future<void> retrieveRating(String shopName) async {
      final response = await http.get(
        Uri.parse('http://10.0.2.2:8000/rating?shopName=${Uri.encodeComponent(shopName)}'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
      );
      if (response.statusCode == 200) {
        print('Rating retrived');
        print(response.body);
        rating = double.parse(jsonDecode(response.body)["rating"]);
      } else {
      ...
      body: Center(
        child: Column(
          children: [
            ...
            ElevatedButton(
                  onPressed: () {
                    retrieveRating(shopNameController.text);
                  },
                  child: Text('Search'),
                ),
